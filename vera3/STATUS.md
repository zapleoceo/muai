...
осмысляет разговор один раз и кладёт **выжимку** в текст события, а
дословную стенограмму — рядом, в `content_extra` (видна в карточке события,
в поиск не попадает). Ключ брокера при этом не покидает сервер.
