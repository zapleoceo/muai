...
1. дословная расшифровка НЕ должна попадать в текст события (`content_text`,
   по нему триаж, поиск и эмбеддинг) — туда идёт выжимка; сама стенограмма
   хранится отдельно в `content_extra` и видна в карточке события;
