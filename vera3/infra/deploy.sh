...
# Замок: два деплоя одновременно ломают стек. Поймано вживую 2026-08-27 —
# ручной прогон столкнулся с прогоном CI, `docker compose up` упал на
# конфликте имени контейнера (vera3-brain-triage-2), и стек остался с
# двадцатью контейнерами в Created и неотвечающим шлюзом. У CI есть своя
# concurrency-группа, но она не знает ни про ручные запуски, ни про второй
# раннер. Замок знает про всех, потому что он на файле.
#
# Полный деплой при занятом замке выходит сразу: ждать нечего, следующий
# push всё равно задеплоит свежее. Проверка ждёт до LOCK_WAIT_S: посреди
# чужого деплоя она увидела бы полустакан и закричала бы ложной тревогой.
LOCK_FILE="${LOCK_FILE:-/var/lock/vera3-deploy.lock}"
LOCK_WAIT_S="${LOCK_WAIT_S:-180}"

# Обёртка на сервере берёт замок РАНЬШЕ нас: она мутирует общий чекаут
# (git fetch + reset --hard) до запуска этого скрипта, и rsync читает
# именно из него — то есть источник надо защищать снаружи, а не здесь.
# Найдено ревью. Если замок уже держит родитель, повторно брать его нельзя:
# flock привязан к описанию открытого файла, второй `exec 9>` создаёт
# НОВОЕ описание, и мы бы заблокировались на замке собственного родителя.
take_lock() {
    if [ "${VERA3_DEPLOY_LOCK_HELD:-}" = "1" ]; then
        return 0
    fi
    # Код 17, а НЕ 15: «замок занят» безопасно пропустить, а «замок не
    # открывается» — сломанная инфраструктура. Смешивать их нельзя, иначе
    # CI, научённый глотать 15, молча перестанет деплоить вообще.
    exec 9>"$LOCK_FILE" || { echo "ПЛОХО: не открыть замок $LOCK_FILE"; exit 17; }
    if [ "$VERIFY_ONLY" -eq 1 ]; then
        if ! flock -w "$LOCK_WAIT_S" 9; then
            echo "ПЛОХО: деплой идёт дольше ${LOCK_WAIT_S}с — проверять нечего"
            exit 16
        fi
        return 0
    fi
    if ! flock -n 9; then
        echo "другой деплой уже идёт (замок $LOCK_FILE) — выхожу, не ломая его"
        exit 15
    fi
}
...
main() {
    ...
    # Миграции деплой НЕ катит (почему — docs/deploy-ops.md), но молчать о
    # ненакатанных не должен: строка в логе CI рядом с выкладкой кода, который
    # их, возможно, ждёт. На вердикт не влияет.
    bash "$TARGET_DIR/scripts/check_migrations.sh"         || echo "ВНИМАНИЕ: миграции расходятся с учётом — см. строки выше, накат через scripts/apply_migration.sh"
