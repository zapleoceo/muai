...
Миграции катятся **руками** через `scripts/apply_migration.sh <файл>`: он
отказывается применять дважды и сам пишет строку в `schema_migrations`
(таблица — миграция 021). Деплой миграции **не катит**, и это решение, а не
забытый шаг.

**Что случилось (замер 13.09.2026).** Учёт на проде кончался на
`025_slack_auth`, файлы в репозитории — до 030:

| версия | реальность | учёт |
|---|---|---|
| 026 `claude_session_queue` | таблица и `ix_claude_queue_status` есть | нет |
| 027 `ix_events_tg_chat_sent` | индекс есть | нет |
| 028 `ix_membership_child` | нет | нет |
| 029 `ix_usage_created_at` | нет | нет |
| 030 pgvector | расширение `vector` есть, колонки `embedding_vec` нет | нет |

026/027 накатывали psql-ом мимо скрипта; 028/029 не накатывал никто, хотя код,
который на них рассчитан (граф, окно в агрегате дашборда), уже работал.
Заметить было нечем: ни деплой, ни монитор учёт не смотрели.

**Почему накат не автоматический.** Рассмотрен вариант «деплой катит новые
идемпотентные миграции через `apply_migration.sh`, а долгие помечены маркером
в файле». Отвергнут:

* Автоматика держится на маркере, который надо не забыть поставить. Забыли —
  DDL с блокировкой уезжает на горячую таблицу (`events` ~430 тыс.,
  `usage_log` 1.15 млн строк) посреди деплоя, без человека рядом. Цена ошибки
  несимметрична: ручной накат опаздывает, автоматический — стопорит запись.
* Миграции здесь неоднородны: 014 шла `CONCURRENTLY` мимо файла, 030 — это
  старт многочасового бэкфила 3.6 ГБ с порядком «накатить → бэкфилить →
  включить». Это операции, а не шаг сборки.
* Упавшая посреди деплоя миграция оставляет выбор между «код выкачен, схема
  нет» и «деплой красный из-за схемы», и оба хуже явного ручного шага.

Вместо этого дрейф сделан **громким**, в двух местах:

1. **Монитор, проверка 12** (`migrations_drift`): `scripts/check_migrations.sh`
   сверяет файлы `infra/migrations/*.sql` с `schema_migrations`. Алерт по
   обычной streak-логике, throttle 720 мин — долг, а не авария минуты, но
   напоминает дважды в сутки, пока не закрыт.
2. **Деплой** (`infra/deploy.sh`): после принятой выкладки печатает то же
   расхождение в лог CI строкой `ВНИМАНИЕ: миграции расходятся с учётом`. На
   вердикт деплоя не влияет.

Для незаписанной миграции проверка дёшево смотрит объекты, которые та создаёт
через `IF NOT EXISTS` (индекс, таблица, расширение, колонка), и пишет одно из:
`объекты есть (N из N), но в учёт не записана` / `применена частично` /
`не применена`. VIEW, DROP и данные не проверяются — такая миграция
называется `не записана в учёт (объекты автоматически не проверяются)`.
В учёт проверка **ничего не пишет**: «объекты есть» ещё не значит «миграция
выполнена целиком», это решает человек. Отдельно ловится строка в учёте без
файла (переименование) и нечитаемый учёт — лежащий postgres не выглядит как
«расхождений нет».

Проверить без побочных эффектов: `vera3-monitor.sh --check-migrations`
(тест `tests/unit/test_monitor_migrations.py` воспроизводит состояние 13.09 на
подставном psql).

**CONCURRENTLY.** Индекс на большой горячей таблице — `CREATE INDEX
CONCURRENTLY` без `BEGIN/COMMIT` (в транзакции он запрещён; `apply_migration.sh`
шлёт файл в psql без `-1`, команды идут по одной). 029 переписана так до
наката. Ловушка: упавшее построение оставляет индекс INVALID, и `IF NOT
EXISTS` при повторе его молча пропустит. Поэтому `apply_migration.sh` после
выполнения файла проверяет `pg_index.indisvalid` по всей базе и при любом
невалидном индексе **не пишет миграцию в учёт** и выходит с ошибкой —
удалить индекс `DROP INDEX CONCURRENTLY` и накатить заново (тест
`test_apply_migration.py`).

Ограничение сверки: `check_migrations.sh` находит объекты только по
неквалифицированным именам без кавычек. Объект `public.x` или `"X"` не будет
посчитан — проверка занизит «есть N из M», но не объявит миграцию
применённой. Имена в миграциях писать без схемы и кавычек.
...
```cron
40 4 * * * docker exec -i vera3-postgres psql -qU vera -d vera < /var/www/vera3/scripts/prune_usage_log.sql >> /var/log/vera-usage-prune.log 2>&1
```

**Строка реально появилась в кроне только 04.09.2026, и до того не чистилось
ничего.** Скрипт написан 01.09 вместе с этим разделом, и оба — и его шапка, и
текст здесь — описывали крон-строку так, будто она есть. Её не было ни в
`crontab`, ни в `/etc/cron.*`, ни в compose: `grep` по имени скрипта не
находил НИ ОДНОГО вызова на всей машине.

Заметить было нечем. Таблице к тому моменту как раз исполнялось 90 дней,
поэтому «строк старше срока: 0» выглядело как исправно работающая ретенция —
а означало, что срок ещё не наступил ни для одной строки.

Мораль общая: написанный скрипт ретенции и запланированная ретенция — разные
вещи, и отличает их ровно одна проверка — кто его вызывает. Признак живой
чистки — её лог (`/var/log/vera-usage-prune.log`), а не отсутствие старых
строк.

Время 40 минут, а не 15: в `15 4` уже стоит `aibroker-prune-jobs.sh`, а обе
задачи ходят в один демон и в соседние базы.
...
## Мёртвый юнит `vera-backfill.service` (снят 04.09.2026)

Systemd-юнит `vera-backfill.service` звал `docker exec … vera-vera-core-1` —
контейнер из дерева Веры 2, удалённого 20.08.2026. С тех пор он падал сразу,
`Restart=always` поднимал его снова через 30 секунд, и так по кругу:
**9868 перезапусков** с последней перезагрузки (аптайм 3.4 суток), то есть
2880 в сутки, каждый — впустую.

Единственным следом были 11 МБ одинаковых строк «No such container» в
`/var/log/vera-brain-backfill.log`, который не попадал под `logrotate.d/vera`
(там перечислены четыре файла поимённо, этого среди них нет).

Проверено перед снятием, что за юнитом не осталось настоящей работы: событий
без вектора 605 из 435 893 (0.14%), и они размазаны с 28.06 по 02.09 ровным
слоем — это отдельные осечки встраивания, а не хвост незаконченного бэкфила.
Вера 3 считает вектора по ходу приёма, отдельный догоняющий процесс ей не
нужен.

Снят `systemctl disable --now`; файл юнита оставлен на месте как след.

**Чем ловить такое впредь:** монитор следит за контейнерами, а не за
