...
**Пачка — один INSERT, не один на событие.** `insert_events()` собирает
многострочный `VALUES`; поллер gmail тянет до `MAX_PER_RUN=500` писем за
прогон, и раньше это были 500 отдельных обращений к базе внутри одной
транзакции. Две детали, которые из этого следуют:

- `RETURNING` отдаёт только реально вставленные строки, но не говорит, какой
  из specs какой, поэтому возвращаются `(id, source, source_event_id)` и
  сопоставление идёт по ключу дедупа. По позиции было бы багом ровно в том
  случае, ради которого всё делается — когда часть пачки уже в базе.
- Специи группируются по НАБОРУ полей: многострочный `VALUES` требует
  одинаковых колонок, а источники заполняют разные. Добивать недостающие
  как `NULL` нельзя — это затрёт server-side дефолты. У одного источника
  форма обычно одна, так что групп почти всегда одна.
...
- **Контейнеру нужен `TOKEN_SECRET`** — тем же ключом дашборд шифрует токен при
  подключении. Пропустишь в compose — строка в `slack_auth` есть, а поллер
  пишет «нечем расшифровать токен». Такую ошибку он НЕ считает порчей строки и
  подключение не гасит: это настройка, а гашение заставило бы вводить токен
  заново без толку. Поймано вживую при первом подключении через UI.
...
- **Нечитаемый канал перестаёт долбиться (12.09.2026).** `Slackbot` — это
  псевдо-личка: Slack отдаёт её в `users.conversations`, но
  `conversations.history` по ней всегда возвращает `channel_not_found`.
  Опрос раз в 5 минут писал из-за этого **240 ERROR в сутки**, и настоящие
  сбои тонули в них. Теперь `slack_channel_unreadable` (в
  `vera_shared.ingest_policy`, рядом с денай-листом) распознаёт
  `channel_not_found` / `not_in_channel` / `is_archived` как «канала для нас
  нет», такой отказ логируется WARNING, а не ERROR, и следующие
  `SLACK_UNREADABLE_RETRY_H` (24) часа канал пропускается целиком
  (`poller.skip_unreadable` — чистая функция, решает по прошлой ошибке и
  времени последней попытки). Не «никогда»: в канал могут вернуть, и тогда
  он оживёт сам на очередной пробе. Гасить `is_active` вместо этого нельзя —
  `upsert_conversations` поднимает флаг обратно на каждом синке, потому что
  Slack по-прежнему отдаёт канал списком. Замерен был только
  `channel_not_found` на `Slackbot`; `not_in_channel` и `is_archived` добавлены
  по смыслу — у обоих `conversations.replies` отказывает так же, как
  `conversations.history`, так что пропуск ничего не теряет. Ради этого же
  `poll_once` вынесен из замыкания `main_loop` в модуль: отсев проверяется
...
## claude_chat (транскрипты Claude Code)

- Контейнера нет: `scripts/claude_chat_sync.py` живёт на ноутбуке — файлы
  `~/.claude/projects/**/*.jsonl` есть только там. Пишет в
  `POST /event/claude_chat`. Отличается от источника `claude`: тот — факты,
  которые агент сохраняет через MCP (`/v1/claude/remember`), а этот — сама
  переписка.
- Курсор — смещение в БАЙТАХ по каждому файлу, в `~/.claude/vera_sync_state.json`.
- Три грабли, все поймано вживую 2026-08-26 (синк не работал ни разу с момента
  появления, 259 событий в мозге были от другого пути):
  1. **Секрет читался только из окружения**, хотя `--setup` создаёт
     `~/.claude/vera_sync.env` и просит его заполнить. Файл не читался никогда →
     каждый запуск давал HTTP 401 («85 errors, 0 posted»). Теперь порядок как у
     слушателя: окружение важнее файла, но файл читается.
  2. **Курсор двигался на размер файла даже при провале отправки** — и это было
     записано в комментарии как намеренное («don't loop forever on bad lines»).
     Один 401 или обрыв сети молча съедал весь хвост навсегда: та же грабля, что
     чинили у gmail с date-granular `after:` и у Trello. Теперь курсор идёт
     только за успешно отправленным; битая строка (не разобрался JSON) —
     ядовитое сообщение, её пропускаем, а сбой транспорта обрывает файл.
  3. **`--dry-run` подменял отправку на успех И сохранял состояние** — то есть
     «посмотреть, что будет» съедало бэклог: следующий настоящий прогон видел
     216 строк вместо 25 069. Теперь сухой прогон курсор не трогает.
- **Cloudflare режет по подписи клиента.** С `User-Agent: Python-urllib/3.12`
  шлюз за Cloudflare отдаёт `403 error code 1010` («banned based on your browser
  signature»); `curl` проходил, поэтому ручная проверка ничего не показывала.
  И скрипт, и слушатель теперь называют себя своим именем
  (`vera-claude-sync/1.0`, `vera-listener/1.0`) — честнее подделки под браузер.
- Время: транскрипты Claude несут метку со зоной (`…Z`), а `events.occurred_at`
  — `timestamp WITHOUT time zone`. asyncpg на tz-aware падал DataError, и шлюз
  отдавал **500 на клиентские данные**. Теперь `RawEvent` нормализует к
  наивному UTC на приёме (это соглашение всего проекта), и скрипт тоже шлёт уже
  наивный — 500 на входных данных это баг сервера, а не клиента.
- **Шлёт по событию на сообщение.** Для 66 файлов это ~11 700 сырых реплик.
  Саммари по сессии вместо сырой переписки — отдельная задача.
...
**Сломанный источник не роняет страницу.** Поймано вживую: `trello_boards`
никогда не накатывали на прод, и запрос к ней уронил всю страницу из
четырнадцати источников пятисоткой. Теперь сбой одного провайдера — и
состояния, и разбивок — превращается в читаемую строку («таблица не создана —
миграция не накатана»), а остальные источники видны. Иначе одна ненакатанная
миграция прячет состояние всех.
