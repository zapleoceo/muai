...
Оставшиеся 4 — настоящие дубли: их email уже закреплён за ДРУГОЙ сущностью
(Yevhenii Pavlenko вообще лежит тремя: gmail + slack + telegram). Скрипт их
печатает и не трогает. Тут важно, что рабочий email **глобально уникален** —
значит это детерминированный дубль, а не догадка, ровно как коллизия
@username в telegram. Слить их — одно решение владельца на
`/entities/duplicates`.
