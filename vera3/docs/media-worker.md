...
похоронены, а лежат в общей очереди и `media_requeue.top_up` подхватывает их
как обычно (раз в три часа). На ближайшем проходе они пойдут в работу, получат
ту же ошибку и деградируют уже правильно — как постоянные. Ручной бэкфилл дал
бы тот же результат на три часа раньше и поэтому не делался.
