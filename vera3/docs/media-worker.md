...
код ВПЛОТНУЮ за ним, через пробел/двоеточие), и постоянным считается **4xx
кроме 429**: 4xx — это плохой запрос, повтор его не исправит; 429 — темп, а не
запрос; 5xx (включая 503 «no provider», где ключи выходят из кулдауна за
минуты) остаются временными. Четвёртый формат записи кода поймается тем же
правилом.

Зазор между маркером и кодом намеренно узкий. Широкий («любые не-цифры в
пределах 20 символов») цеплял число из ТЕЛА сообщения: `whisper: file is 413
seconds long` — обычный длинный войс — читался как HTTP 413 и уезжал в
`media_permanent` навсегда. Во всех четырёх реальных форматах код стоит
вплотную к маркеру, в тексте сообщения — никогда; на это есть тест
`test_number_from_the_message_body_is_not_a_status_code`.

**Уже застрявшие события чинятся сами, отдельного UPDATE нет.** У четырёх
событий из замера `media_permanent` остался `false` — то есть они не
похоронены. `media_requeue.top_up` берёт ровно такие строки: `media_recognition
= 'failed'`, `triage_status = 'done'`, `media_permanent <> 'true'`, не канал и
проходящие политику участия владельца (`scripts/media_requeue.py`). Именно
этот цикл и гонял их по кругу все 48 часов замера, так что подхват доказан
самим багом. На ближайшем проходе они пойдут в работу, получат ту же ошибку и
деградируют уже правильно — как постоянные.

Проверка 17.09.2026 перед мерджем: строк с `media_recognition = 'failed'`,
`media_permanent <> 'true'` и кодом брокера в тексте ошибки на проде **ноль** —
цикл успел прокрутить их ещё раз. Всего в восстановимом состоянии 2 950 строк
(против 7 942 уже помеченных постоянными), все с `triage_status = 'done'`.
Ручной бэкфилл дал бы тот же результат на три часа раньше и поэтому не
делался.
...
3. `top_up` — доливает из восстановимых провалов, голосовые вперёд, дальше
   ...
   **С 23.09.2026 берёт и то, что распознавать не пытались вовсе** —
   `media_recognition` отсутствует, а `media_skip_reason` пуст. До этого
   запрос начинался с `media_recognition = 'failed'`, и непробованное было
   невидимо в принципе: очередь стояла на нуле, доливка добирала 0, а 499
   подходящих медиа (396 фото, 102 голосовых, 1 аудио) лежали месяцами. Пустой
   `media_skip_reason` обязателен: уборка (шаг 1) метит выкинутое именно им,
   и без условия доливка тянула бы выкинутое обратно тем же проходом. Вид
   медиа в SQL берётся из `media_policy.RECOGNIZED_MEDIA_KINDS`, а не
   перечисляется заново: иначе стикеры и документы без метки забили бы окно.
   Это не теоретически: среди непробованного было ещё 1 166 записей видов,
   которые не распознаются вовсе (кружки `video_note` 754, опросы 225,
   геометки 148, контакты), — ручной подсчёт, отсекавший лишь четыре вида,
   сначала и дал завышенные 1 665 вместо настоящих 499.
...
4. `measure` — считает, сколько медиа политика пропускает ВСЕГО и сколько из
   ...
   (`media_backlog_total` / `media_backlog_left`). Агрегация идёт по ЧАТУ, а не
   по событию — политика решает по чату, поэтому это сотня строк, а не десятки
   тысяч.

   **Остаток дашборд считает сам, вживую** — `vera_shared.media_backlog`
   (`unrecognized_left`), тем же кодом, каким его считает `measure`, с кэшем на
   30 секунд. Запись крона осталась запасным вариантом на случай ошибки запроса.
   Раньше страница показывала только запись трёхчасового прогона, и 26.09.2026
   это дало «1 осталось» при пустой очереди: файл распознали, а число ждало
   следующего крона. Считается только НЕраспознанное — это тысячи строк, под
   которые стоит частичный индекс `ix_events_media_unrecognized`
   (миграция 034); предикат индекса обязан совпадать с запросом в модуле.
   `media_backlog_total` по-прежнему из крона: полный проход по `events` стоит
   4 секунды, а число меняется медленно.
   **Остаток не включает `media_permanent`** — медиа, которых больше нет в
   Telegram. До 23.09.2026 включал: дашборд показывал 6 511, из них 6 012
   распознать нельзя никогда, и число не могло опуститься ниже ~6 000 —
   при пустой очереди это выглядело как вечный затык.
