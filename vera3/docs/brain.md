...
- **Дедуп `/v1/claude/remember`**: косинус по halfvec в SQL для перелитых
  ...
  десятки. Размерность в касте `halfvec(N)` читается из `vectors.VEC_DIMS`
  в момент вызова, а не импортом имени: импортированная константа замерзает
  при загрузке модуля, и 13.09.2026 интеграционный тест, подменяющий
  размерность на 3, уронил CI («expected 1024 dimensions, not 3»). Запись эмбеддинга — одна функция `vectors.embedding_upsert` для
