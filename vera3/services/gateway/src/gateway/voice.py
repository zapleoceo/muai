...
Ключевое свойство: в ТЕКСТ события (`content_text` — его читают триаж, поиск
и эмбеддинг) уходит выжимка — с кем, через что, о чём, решения,
договорённости, цифры плюс цитаты там, где важна формулировка. Дословная
стенограмма с авторством реплик хранится рядом, в `content_extra`
(`transcript_record`), и видна только в карточке события `/events/{id}`: в
вектор и в поиск она не попадает. До коммита 51cab013 стенограмма не
сохранялась вовсе и оставалась только на ноутбуке.
