...
async def cooling_s() -> float:
    """Секунд до конца кулдауна пула, без которого работать нельзя.
    ...
    Смотрим только на пул окон: без него работы нет вообще. Закрытый умный пул
    работу не останавливает — сборку в этом случае доводит тот же дешёвый пул.
    """
    return await llm_cooldown_remaining_s(SPEC.part_capability)
