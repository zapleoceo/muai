...
#: Поэтому сопоставляем по СМЫСЛУ (код), а не по написанию: слово-маркер и
#: трёхзначный код СРАЗУ за ним (между ними только пробел/двоеточие) — так же
#: ловится и четвёртый формат, если появится. Зазор намеренно узкий: широкий
#: («любые не-цифры в пределах 20 символов») цеплял число из ТЕЛА сообщения, и
#: `whisper: file is 413 seconds long` — обычный длинный файл — уезжал в
#: `media_permanent` навсегда. Код ответа стоит вплотную к маркеру во всех
#: четырёх реальных форматах, тело сообщения — никогда.
_STATUS_RE = re.compile(
    r"\b(?:http|https|broker|whisper|vision|poll|status|code)\b[\s:=-]{0,3}(\d{3})\b",
    re.IGNORECASE,
)
