...
export async function loadEmbedder() {
  ...
  const lastRun = d.last_run ? new Date(d.last_run).toLocaleString('ru') : (d.running ? 'в процессе (первый запуск)' : 'ещё не запускался');
  ...
  box.innerHTML = `
    <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin-bottom:12px">
      ...
      <div>Последний запуск: <b>${lastRun}</b></div>
    </div>
    <div style="font-size:.9rem;color:#94a3b8">Топ чатов по чанкам:</div>
    <div style="margin-top:6px">${(d.per_chat || []).slice(0, 10).map(c => `
      <div class="bar-row">
        <div class="name" title="${esc(c.title || c.chat_id)}">${esc(c.title || String(c.chat_id))}</div>
        <div class="bar-wrap"><div class="bar" style="width:${Math.round(c.chunks / Math.max(...(d.per_chat || [{ chunks: 1 }]).map(x => x.chunks)) * 100)}%"></div></div>
        <div class="bar-count">${c.chunks}</div>
      </div>`).join('')}</div>
    ${errHtml}`;
}
