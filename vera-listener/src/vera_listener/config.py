...
class Config:
    ...
    stt_device: str = "NPU"
    #: Язык распознавания. `auto` — модель решает сама на каждом куске.
    #:
    #: Раньше здесь был жёсткий `ru`, и это молча портило украинскую речь:
    #: замер 17.09 на одном и том же звуке дал 34% совпадения с эталоном при
    #: принудительном русском и 61% при украинском, а одна фраза при `ru`
    #: выродилась в английский мусор («You разработive concept»). В команде
    #: говорят на двух языках, так что один язык на всех — заведомо неверен
    #: для половины участников.
    #:
    #: Авто не хуже принуждения там, где язык угадан верно: на русской речи
    #: `auto` и `ru` дают слово в слово одно и то же, в том числе на коротком
    #: тихом куске (замер там же). Ставить конкретный язык по-прежнему можно —
    #: `VERA_LANGUAGE=ru`, — это осмысленно, если говорят строго на одном.
    #:
    #: Ответу модели `en` (и языку не из ru/uk/en/id) при `auto` не верим без
    #: перепроверки: 24.09.2026 весь день русская речь уходила английским
    #: переводом. Как проверяем и сколько это стоит — `language.py`.
    language: str = "auto"
