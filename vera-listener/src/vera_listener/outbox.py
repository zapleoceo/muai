...
class Outbox:
    ...
    def recover(self, max_age_s: float = 60.0,
        ...
        пропускались, второго прохода не было, и они копились — 2026-08-27 в
        open/ лежало пять брошенных сессий возрастом от восьми минут.

        Минута осталась как защита от гонки с только что умершим процессом,
        который мог не докончить строку. Но 17.09 она подвела ровно тем же
        способом: слушатель перезапустили ПОСРЕДИ разговора, файл писался до
        последней секунды, при старте оказался моложе минуты — и был пропущен.
        Второго захода не было, и 1281 реплика часового созвона осталась лежать
