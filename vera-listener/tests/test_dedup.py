...
class TestKnownResidual:
    """Пара, которую фильтр ОБЯЗАН оставить как есть — и это не остаток эха.

    Дословная пара из записи 31.08 (не сокращённая — нашло ревью, что
    прежняя версия текста в тесте отличалась от файла и давала другое число).
    Выглядит как парафраз: «да я как админ у меня должно быть два пути…» /
    «Я как админ, смотри, у меня должно быть два пути». По словам (Jaccard
    0.50) действительно похоже на эхо — ниже порога 0.60 совсем немного.

    Но решает не цифра, а ПОРЯДОК ВРЕМЕНИ: mic звучит в 335.1с, system — в
    338.6с, на 3.5с ПОЗЖЕ. Эхо физически не может обогнать источник — звук
    идёт от динамика в микрофон, а не наоборот. Значит это Дима формулирует
    мысль первым, а Оля через три секунды подхватывает и договаривает своими
    словами — обычный ход разговора, а не эхо. Порог 0.60 (не 0.50) существует
    отчасти ради этого случая тоже: числом эту пару можно спутать с реальным
    эхом (см. `TestWordOverlapCatchesMidWordDrops`, где Jaccard 0.60 ловит
    настоящую пару), и снижать его — значило бы стирать слова Димы.

    Остаётся честный предел: пересказ, который по тексту неотличим от
    настоящего эха с иным словарём. Таких пар около десятка на запись —
    лечится только опознанием ГОЛОСА, это отдельная задача.
    """

    def test_independent_dialogue_is_not_flagged_as_echo(self):
        utterances = [
            _u(338.6, "system", "Я как админ, смотри, у меня должно быть два пути"),
            _u(335.1, "mic", "да я как админ у меня должно быть два пути "
                             "мои студенты которые изучают курсы должна быть "
                             "автопроверка"),
        ]
        assert len(drop_echo(utterances)) == 2


class TestWordOverlapCatchesMidWordDrops:
    """Третий сигнал: доля общих слов, а не подстрока символов.

    Реальная пара из записи (31.08, 347.7с): loopback вставляет «прям» в
    середину фразы, которую мик поймал без него. Непрерывная подстрока рвётся
    на месте вставки, обычное сходство падает до 0.735 (порог 0.75) — оба
    прежних сигнала мимо. По словам пересечение остаётся почти полным.
    """

    def test_word_inserted_in_the_middle_is_still_caught(self):
        utterances = [
            _u(0.0, "system",
               "А, например, какие-то супердорогие курсы, я хочу там прям, "
               "чтобы я сама проверяла, давала им экспертную оценку их "
               "ответов, домашек."),
            _u(1.0, "mic",
               "А, например, какие-то супердорогие курсы, я хочу там, "
               "чтобы я сама проверяла,"),
        ]
        marked = mark_echo(utterances)
        mic = next(u for u in marked if u["stream"] == "mic")
        assert mic["echo"] is True

    def test_below_threshold_is_not_caught(self):
        """Порог реален, а не декоративный: чуть меньше — уже не эхо."""
        mic = "какие-то супердорогие курсы, я хочу там, чтобы я сама"
        system = "супердорогие ответов домашек экспертную оценку прям хочу там"
        assert word_overlap(mic, system) < JACCARD_THRESH
        assert not looks_like_echo(mic, system)

    def test_full_sequence_matcher_ratio_is_below_threshold_here(self):
        """Доказывает, что случай реально требует третьего сигнала, а не
        совпадает по прежним двум — иначе тест выше ничего бы не проверял."""
        mic = ("А, например, какие-то супердорогие курсы, я хочу там, "
              "чтобы я сама проверяла,")
        system = ("А, например, какие-то супердорогие курсы, я хочу там прям, "
                  "чтобы я сама проверяла, давала им экспертную оценку их "
                  "ответов, домашек.")
        assert similar(mic, system) < 0.75
        short, long = sorted((normalize(mic), normalize(system)), key=len)
        assert short not in long


class TestWordOverlap:
    def test_identical_bag_of_words(self):
        assert word_overlap("привет как дела", "дела как привет") == 1.0

    def test_no_shared_words(self):
        assert word_overlap("привет мир", "пока земля") == 0.0

    def test_empty_is_zero(self):
        assert word_overlap("", "что угодно") == 0.0
        assert word_overlap("что угодно", "") == 0.0

    def test_short_lines_can_still_score_high_but_are_gated_by_length(self):
        """word_overlap сам по себе не знает о минимуме длины — это работа
        looks_like_echo. Голая функция честно даёт 1.0 на «да»/«да»."""
        assert word_overlap("да", "да") == 1.0
