...
_ROUTER_POLICIES = (
    ...
    "   - 'сколько я отправил сообщений' → filters=[direction=out], select=[{field:message_id, agg:COUNT, as_name:messages}]\n"
    "   - 'топ чатов по активности' → group_by=[chat_id,chat_title], select=[chat_id,chat_title,{message_id,COUNT,messages}], order_by=[messages desc]\n"
    "   ВАЖНО: для подсчёта сообщений используй field:'message_id' с agg:'COUNT'. Поля 'message_count', 'count' не существуют.\n"
